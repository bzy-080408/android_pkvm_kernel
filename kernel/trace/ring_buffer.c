...
static void ring_buffer_update_view(struct ring_buffer_per_cpu *cpu_buffer)
{
	...
	err = cpu_buffer->buffer->ext_cb->update_footers(cpu_buffer->cpu);
	if (err)
		goto unlock;
...
#define TRACE_BUFFER_PACK_HDR_SIZE offsetof(struct trace_buffer_pack, cpu_pack)
...
{
	struct ring_buffer_pack *cpu_pack = &pack->cpu_pack[0];
	int i, j;
	...
	for (i = 0; i < trace_buffer->cpus; i++) {
		...
		cpu_pack = next_ring_buffer_pack(cpu_pack);
	}
}
