...
static void init_shadow_structs(void *shadow_addr, const struct kvm *kvm)
{
	...
	for (i = 0; i < kvm->created_vcpus; i++) {
		...
		struct shadow_vcpu_state *shadow_vcpu_state = &shadow_vcpu_states[i];

		memcpy(&shadow_vcpu_state->ctxt, vcpu_ctxt, sizeof(shadow_vcpu_state->ctxt));
		memcpy(&shadow_vcpu_state->hyp_state, hyps, sizeof(shadow_vcpu_state->hyp_state));
	}
}
 ...
 */
static void update_vcpu_state(struct kvm_cpu_context *vcpu_ctxt,
			      struct vcpu_hyp_state *vcpu_hyps,
			      int shadow_handle)
{
	hyp_state_shadow_handle(vcpu_hyps) = shadow_handle;
...
static int create_el2_shadow(struct kvm *kvm)
{
	...
	for (i = 0; i < kvm->created_vcpus; i++) {
		struct kvm_cpu_context *vcpu_ctxt = &vcpu_ctxt(kvm->vcpus[i]);
		struct vcpu_hyp_state *hyps = &hyp_state(kvm->vcpus[i]);

		update_vcpu_state(vcpu_ctxt, hyps, shadow_handle);
	}
