...
{
	gpa_t gpa = vcpu_get_reg(vcpu, 1);
	gpa_t size = PAGE_SIZE;
	bool is_private = (nr == ARM_SMCCC_KVM_FUNC_MEM_UNSHARE);
	int ret;

	ret = kvm_vm_ioctl_set_mem_attr(vcpu->kvm, gpa, size, is_private);
	if (ret)
		return ret;
