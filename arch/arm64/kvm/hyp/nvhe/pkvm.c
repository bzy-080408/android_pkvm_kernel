...
/*
 * Gets the hyp shadow state (vm, cpu context, and hyp state) for the
 * corresponding vcpu.
 *
 * If successful, returns true and sets vm_ptr, ctxt_ptr, and hyps_ptr to point
 * to the respective shadow states.
			       ...
			       struct vcpu_hyp_state **hyps_ptr)
{
	...
	if (unlikely(check_memory_addr((u64)vcpu, sizeof(*vcpu))))
		return false;

	shadow_handle = vcpu_shadow_handle(vcpu);
	...
	if (vm_ptr)
		*vm_ptr = shadow_vcpu_state->vm;

	if (ctxt_ptr)
		*ctxt_ptr = &shadow_vcpu_state->ctxt;

	if (hyps_ptr)
		*hyps_ptr = &shadow_vcpu_state->hyp_state;
 ...
 * Return 0 on success, negative error code on failure.
 */
static int init_shadow_structs(const struct kvm *kvm,
			       void *shadow_addr,
			       size_t size,
			       int shadow_handle)
{
	...
	vm->mmu = kern_hyp_va(&((struct kvm *)kvm)->arch.mmu);
	...
	for (i = 0; i < num_vcpus; i++) {
		...
		shadow_state->vm = vm;
		hyp_state_shadow_handle(&shadow_state->hyp_state) =
			shadow_handle;
	}
 ...
 * negative error code on failure.
 */
int __pkvm_init_shadow(const struct kvm *kvm,
		       void *shadow_va,
		       size_t shadow_size)
{
