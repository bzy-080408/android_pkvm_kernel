...
static void unpin_host_vcpu(struct kvm_shadow_vcpu_state *shadow_vcpu_state)
{
	...
	if (sve_state) {
		size_t sve_state_size = vcpu_sve_state_size(shadow_vcpu);
			    ...
			    int vcpu_idx)
{
	...
	shadow_vcpu->arch.mp_state.mp_state = KVM_MP_STATE_STOPPED;
...
/*
 * Return the size of the memory area for tracking the last vcpu that has run a
 * physical cpu on this vm.
 */
static size_t get_last_ran_size(void)
{
		       ...
		       unsigned long pgd_hva, unsigned long last_ran_hva)
{
	...
	void *last_ran = NULL;
	unsigned int nr_vcpus;
	size_t shadow_size, pgd_size, last_ran_size;
	void *pgd = NULL;
	int ret;
	...
	last_ran_size = get_last_ran_size();
	...
	init_shadow_vm(kvm, vm, last_ran, last_ran_size, nr_vcpus);
...
/*
 * Initialize the shadow copy of the protected vcpu state in memory previously
 * donated by the host.
 *
...
int __pkvm_teardown_shadow(unsigned int shadow_handle)
{
	...
	for (i = 0; i < nr_vcpus; i++)
		teardown_donated_memory(mc, vm->shadow_vcpu_states[i],
					sizeof(vm->shadow_vcpu_states[i]));
