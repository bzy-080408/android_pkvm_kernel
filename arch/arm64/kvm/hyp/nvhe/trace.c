...
spin:
	...
	/*
	 * The overrun is synchronized with the reader swap. It must therefore
	 * be updated here and not in rb_update_footers()
	 */
...
{
	unsigned long entries, pages_touched;
	struct rb_ext_page_footer *footer;
...
static int rb_page_init(struct hyp_buffer_page *bpage, unsigned long hva)
{
	...
	ret = pkvm_create_mappings(hyp_va, hyp_va + PAGE_SIZE, prot);
	if (ret)
		return ret;
...
static void rb_cpu_teardown(struct hyp_rb_per_cpu *cpu_buffer)
{
	...
	while (atomic_cmpxchg_relaxed(&cpu_buffer->status, HYP_RB_READY, HYP_RB_UNUSED)
		== HYP_RB_WRITE)
	;
	...
	for (i = 0; i < cpu_buffer->nr_pages; i++) {
		...
		pkvm_remove_mappings((void *)bpage->page, (void *)bpage->page + PAGE_SIZE);
	}
}
...
{
	enum kvm_pgtable_prot prot = pkvm_mkstate(PAGE_HYP_RO, PKVM_PAGE_SHARED_OWNED);
	...
	ret = pkvm_create_mappings((void *)pack, (void *)(pack + pack_size), prot);
	if (ret)
		goto err;
	...
	rb_pack = &trace_pack->cpu_pack[0];
	for (i = 0; i < trace_pack->nr_cpus; i++) {
		struct hyp_rb_per_cpu *cpu_buffer;
		int cpu = rb_pack->cpu;
		...
		rb_pack = next_ring_buffer_pack(rb_pack);
	}

	hyp_clock_setup(pack->epoch_ns, pack->epoch_cyc);
err_remove_mappings:
	pkvm_remove_mappings((void *)pack, (void *)(pack + pack_size));
	if (ret)
		__pkvm_stop_tracing_locked();
err:
	hyp_spin_unlock(&trace_rb_lock);
