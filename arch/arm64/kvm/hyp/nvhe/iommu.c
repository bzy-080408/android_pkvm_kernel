...
/* list of registered IOMMU drivers */
...
static bool driver_id_unique(struct pkvm_iommu_driver *drv)
{
...
int __pkvm_iommu_driver_init(int id, void *data, size_t size)
{
	...
	if (!ret)
		driver_release_init(drv, true);

	host_unlock_component();
out_release:
	if (ret)
		driver_release_init(drv, false);
out_unlock:
