...
static void mem_transition_lock(const struct pkvm_mem_transition *tx)
{
	...
	if (tx->initiator.id == PKVM_ID_GUEST)
		guest_lock_component(tx->initiator.guest.vcpu);
...
fail:
	for (; start < addr; start += PAGE_SIZE)
		hyp_page_unlock(hyp_phys_to_page(start));
	return -EPERM;
}

static void __host_unlock_pages(u64 addr, u64 size)
{
	u64 end = addr + size;

	for (; addr < end; addr += PAGE_SIZE)
		hyp_page_unlock(hyp_phys_to_page(addr));
}
				       ...
				       enum pkvm_page_state state)
{
	...
	for (; addr < end; addr += PAGE_SIZE)
		host_setstate(hyp_phys_to_page(addr), state);
	__host_unlock_pages(start, size);

	return 0;
}

static void host_cancel_transition(const struct pkvm_mem_transition *tx)
{
	u64 size = tx->nr_pages * PAGE_SIZE;
	u64 addr = tx->initiator.addr;

	__host_unlock_pages(addr, size);
}
...
static int do_share(struct pkvm_mem_share *share)
{
	...
	mem_transition_lock(&share->tx);
	ret = check_share(share);
	if (!ret)
		ret = WARN_ON(__do_share(share));
	mem_transition_unlock(&share->tx);
...
static int do_donate(struct pkvm_mem_donation *donation)
{
	...
	mem_transition_lock(&donation->tx);
	ret = check_donation(donation);
	if (!ret)
		ret = WARN_ON(__do_donate(donation));
	mem_transition_unlock(&donation->tx);
