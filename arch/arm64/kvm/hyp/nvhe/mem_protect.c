...
int kvm_guest_prepare_stage2(struct kvm_shadow_vm *vm, void *pgd)
{
	...
	__guest_lock(vm);
	ret = __kvm_pgtable_stage2_init(mmu->pgt, &vm->arch, &vm->mm_ops, 0,
					    ...
					    enum pkvm_page_state state)
{
	...
	struct kvm_protected_vcpu *pkvm = &vcpu->arch.pkvm;
	struct kvm_shadow_vm *vm = pkvm->shadow_vm;
	struct kvm_hyp_memcache *mc = &pkvm->memcache;
	u64 size = tx->nr_pages * PAGE_SIZE;
	u64 addr = tx->initiator.addr;
	enum kvm_pgtable_prot prot;
	phys_addr_t phys;
...
static int check_share(struct pkvm_mem_share *share)
{
	...
	switch (tx->completer.id) {
	case PKVM_ID_HOST:
		ret = host_ack_share(completer_addr, tx, share->prot);
		break;
	case PKVM_ID_HYP:
...
static int __do_share(struct pkvm_mem_share *share)
{
	...
	switch (tx->completer.id) {
	case PKVM_ID_HOST:
		ret = host_complete_share(completer_addr, tx, share->prot);
		break;
	case PKVM_ID_HYP:
