...
{
	if (WARN_ON(state & PKVM_NOPAGE))
		return 0ULL;

	prot &= ~PKVM_PAGE_STATE_PROT_MASK;
	prot |= FIELD_PREP(PKVM_PAGE_STATE_PROT_MASK, FIELD_GET(__PAGE_STATE_RES_MASK, state));
...
{
	return FIELD_PREP(__PAGE_STATE_RES_MASK, FIELD_GET(PKVM_PAGE_STATE_PROT_MASK, prot));
}
