...
static int hyp_trace_pipe_open(struct inode *inode, struct file *file)
{
	...
	ring_buffer_poke(iter->trace_buffer, iter->cpu);
	INIT_DELAYED_WORK(&iter->poke_work, __poke_reader); /* TODO: make it global */
...
int init_hyp_tracefs(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		d = tracefs_create_file("trace_pipe_raw", 0600, per_cpu_subdir,
				(void *)cpu,
				&hyp_trace_raw_fops);
		if (!d)
			pr_warn("Failed create hyp/per_cpu/cpu%lu/trace_pipe_raw\n", cpu);
	}
