...
void kvm_flush_remote_tlbs(struct kvm *kvm)
{
	...
	if (is_protected_kvm_enabled())
		kvm_call_hyp_nvhe(__pkvm_tlb_flush_vmid, kvm->arch.pkvm.shadow_handle);
	else
...
{
	return rb_find_add(&ppage->node, &kvm->arch.pkvm.pinned_pages, cmp_ppages) ? -EEXIST : 0;
}
...
int kvm_handle_guest_abort(struct kvm_vcpu *vcpu)
{
	...
	if (is_protected_kvm_enabled() && fault_status != FSC_PERM)
				   ...
				   enum kvm_mr_change change)
{
	...
	if (is_protected_kvm_enabled()) {
		...
		if ((change == KVM_MR_DELETE || change == KVM_MR_MOVE) &&
		    kvm->arch.pkvm.shadow_handle && kvm->arch.pkvm.enabled) {
			return -EPERM;
		}
