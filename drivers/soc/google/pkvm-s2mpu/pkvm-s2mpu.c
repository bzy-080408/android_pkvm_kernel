...
static int s2mpu_probe(struct platform_device *pdev)
{
	...
	ver = readl_relaxed(data->base  + 0x60);
	...
	// if EL2 driver is loaded, register the driver with the EL2 driver
	if (el2_loaded) {
...
static int __init s2mpu_init(void)
{
	//default value if el2 is not loaded
	int ret = -EPERM;

	if (is_protected_kvm_enabled())
		ret = pkvm_load_el2_module(__kvm_nvhe_s2mpu_hyp_init);

	el2_loaded = !ret;

	if (ret)
		pr_err("Can't load s2mpu el2 module, moving control to kernel %d\n", ret);
	else
		pr_info("S2MPU loaded successfully in el2\n");
	return platform_driver_register(&s2mpu_driver);
}
static void __exit s2mpu_exit(void)
{
	platform_driver_unregister(&s2mpu_driver);
}
module_init(s2mpu_init);
module_exit(s2mpu_exit);
