...
/*********** Abstract mod_ops usage by macros so it easy to direct later *************/
///// mod_ops reference //////
static const struct pkvm_module_ops *mod_ops;
#define CALL_FROM_OPS(fn, ...) mod_ops->fn(__VA_ARGS__)
////////////////////////////

//function used explicitly from this file can be just macros
#define __pkvm_hyp_donate_host(x, y)  CALL_FROM_OPS(pkvm_hyp_donate_host, x, y)
#define __pkvm_host_donate_hyp(x, y)	 CALL_FROM_OPS(pkvm_host_donate_hyp, x, y)
//override some macros
#ifdef __hyp_pa
  #undef  __hyp_pa
#endif
#define __hyp_pa(x) CALL_FROM_OPS(module_hyp_pa, x)
#ifdef kern_hyp_va
  #undef kern_hyp_va
#endif
#define  kern_hyp_va(x) CALL_FROM_OPS(module_kern_hyp_va, x)

void dcache_clean_inval_poc(unsigned long start, size_t sz)
{
	CALL_FROM_OPS(flush_dcache_to_poc, (void *)start, sz);
}
...
static int s2mpu_init(void *data, size_t size)
{
	...
	for_each_gb(gb) {
		...
		smpt = (u32 *)kern_hyp_va((phys_addr_t)in_mpt.fmpt[gb].smpt);
		pa = __hyp_pa(smpt);
...
//check if ops passed from pKVM match our usage, each time we use a function it should be added here
//to be checked once at init
static bool are_ops_valid(const struct pkvm_module_ops *ops)
{
	if (!ops || !ops->register_iommu_driver || !ops->module_kern_hyp_va  ||
	    !ops->module_hyp_pa || !ops->flush_dcache_to_poc ||   !ops->memcpy ||
	    !ops->pkvm_hyp_donate_host || !ops->pkvm_host_donate_hyp || !ops->memset){
		return false;
	}
	return true;
}
int s2mpu_hyp_init(const struct pkvm_module_ops *ops)
{
	...
	ret = are_ops_valid(ops);
	if (!ret)
		return -EINVAL;
	//take a reference so we can use it later
	mod_ops = ops;
	ret = ops->register_iommu_driver(&pkvm_s2mpu_driver);
	if (ret)
		return ret;
	return  ops->register_iommu_driver(&pkvm_sysmmu_sync_driver);
